export const PLACEHOLDERS = {
  CHAT_INPUT: '메시지를 입력하세요',
  CHAT_INPUT_FINISHED: '종료된 상담입니다.',
  CLIENT_SEARCH: '고객 아이디를 검색하세요',
  CHAT_MEMO: '상담 중 메모를 입력하세요',
  MEMO_INPUT: '상담 내용이나 특이사항을 입력하세요',
};

export const CHAT_LIST_PLACEHOLDERS = {
  NO_SEARCH_RESULT: '일치하는 고객 아이디가 없습니다.',
  NO_CHAT_RESULT: '상담 목록이 없습니다.',
  NO_CHAT_RESULT_SEARCH: '아이디를 다시 확인해주세요.',
};

export const LOGIN_PLACEHOLDERS = {
  ID_INPUT: '아이디를 입력해주세요',
  PW_INPUT: '비밀번호를 입력해주세요',
};

export const SIGNUP_PLACEHOLDERS = {
  ID_INPUT: '아이디 (4~12자, 영문·숫자)',
  PW_INPUT: '비밀번호 (8~20자, 영문·숫자·특수문자 포함)',
  PW_CONFIRM_INPUT: '비밀번호를 다시 입력해주세요',
  NAME_INPUT: '이름을 입력해주세요',
  PHONE_INPUT: '전화번호 (숫자만 입력)',
  EMAIL_INPUT: '이메일 주소 (예: example@example.com)',
};
